<template>
    <section>
        <el-row>
            <el-col :span='6'>
                <el-button @click='toShowDialog(0)' type='success' size='small' plain>
                    <i class='el-icon-plus' style='margin-right: 10px'></i>新建账号</el-button>
                <el-button @click='uploadTxt(1)' type='warning' size='small' plain>
                    <i class='el-icon-tickets' style='margin-right: 10px'></i>导入TXT文件</el-button>
                <el-button v-if='tableName == "nochannel"' @click='toShowDialog(2)' type='primary' size='small'>
                    <i class='el-icon-more'></i>批量分配渠道</el-button>
            </el-col>
            <el-col :offset='12' :span='6'>
                <el-input v-model='searchMsg' placeholder="账号名称搜索" size='small' clearable @keyup.native.enter='searchAccount'>
                    ...
                    <el-button slot='append' @click='searchAccount'>
        ...
        <el-table :data='currentList' ref="multipleTable" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
            <el-table-column v-if='tableName == "nochannel"' prop="id" type="selection" width="55" align="center">
            </el-table-column>
            <el-table-column prop="name" align="center" label="账户名" width="220">
            </el-table-column>
            <el-table-column prop="pwd" align="center" label="密码" show-overflow-tooltip>
            </el-table-column>
            <el-table-column prop="channelname" align="center" label="所属渠道" show-overflow-tooltip>
            </el-table-column>
            <el-table-column prop="channelcode" align="center" label="渠道码" show-overflow-tooltip>
            </el-table-column>
            <el-table-column prop="descs" align="center" label="说明" show-overflow-tooltip>
            </el-table-column>
            <el-table-column prop="createtime" align="center" label="创建时间" show-overflow-tooltip>
            </el-table-column>
            <el-table-column prop="updtime" align="center" label="修改时间" show-overflow-tooltip>
            </el-table-column>
        ...
        <el-pagination v-if='pager.recordCount > 15 && tableName == "channel"' style='text-align: center; margin-top: 10px;' :total="pager.recordCount" :page-size="pager.pageSize" :current-page="pager.pageNumber" layout="prev, pager, next" @current-change='handlePage'>
        </el-pagination>
...
export default {
    ...
    methods: {
        ...
        setAccountList(data) {  //区别开已分配或未分配渠道
            ...
            this.noChannelList = []
            data.forEach((i) => {
                if (!i.channelcode) {//渠道码为空
                    i.channelcode = ''
                    i.channelid = ''
                    i.channelname = ''
                    this.noChannelList.push(i)
                    this.accountList.push(i)//无渠道数据
                }
                else {
                    this.accountList.push(i)//有渠道的数据
            ...
            if (this.tableName == 'channel') {
                ...
                this.currentList = this.accountList//所有的数据
            }
            else {
        ...
        handlePage(e) {
            ...
            console.log(this.chosedChannelId)
    ...
    created() {
        this.getData(this.chosedChannelId)
    }
}
</script>

    ...
    <div>
        <div class='top-content'>
            <el-input placeholder="请输入搜索关键词..." clearable v-model="searchKey" style='width:800px;' size="small" @keyup.enter.native="getSearch">
                <el-select v-model='obj.companyId' placeholder="公司名" style='width: 140px; margin-right: 20px;' slot='prepend' clearable @change="getChannels('parent')">
                    <el-option v-for="item in channelstree" :key="item.id" :label="item.key" :value="item.id">
                    </el-option>
                </el-select>
                <el-select v-model='obj.pChannelId' placeholder="父渠道" style='width: 140px; margin-right: 20px;' slot='prepend' clearable @change="getChannels('child')" :disabled="obj.companyId.length == 0">
                    <el-option v-for="item in pChannelList" :key="item.id" :label="item.key" :value="item.id">
                    </el-option>
                </el-select>
                <el-select v-model='obj.cChannelId' placeholder="子渠道" style='width: 140px; margin-right: 20px;' slot='prepend' clearable :disabled="obj.pChannelId.length == 0">
                    <el-option v-for="item in cChannelList" :key="item.id" :label="item.key" :value="item.id">
                    </el-option>
                </el-select>
                <el-select v-model='searchId' placeholder="字段名" style='width: 100px;' slot='prepend' clearable>
                    <el-option label="订单号" value="1"></el-option>
                    <el-option label="游戏名称" value="2"></el-option>
                </el-select>
                <el-button slot="append" icon="el-icon-search" @click='getSearch'></el-button>
            </el-input>
            <el-date-picker v-model="searchTime" size='small' type="daterange" value-format="yyyy-MM-dd" @change='timeChange' start-placeholder="开始日期" end-placeholder="结束日期">
            </el-date-picker>
        </div>
        <div>
            <el-table :data='payList' border v-loading='loading'>
                <el-table-column prop='userid' label='用户ID' align='center' width='80'>
                    <template slot-scope="scope">
                        <div>
                            {{scope.row.userid}}
                            <el-popover placement="right" :width="popWid" trigger="click">
                                <div>
                ...
                <el-table-column v-for='(item, index) in colList' :key='index' :width='item.width' :prop='item.prop' :label='item.label' align="center">
                </el-table-column>
            ...
            <el-pagination style='text-align: center; margin-top: 10px;' v-if='pager.recordCount > 15' :current-page="pager.pageNumber" layout="prev, pager, next" :page-size="pager.pageSize" :total="pager.recordCount" @current-change='handlePage'>
            </el-pagination>
        </div>
    </div>
</template>
